...
- ✅ Consistent developer commit patterns
- ✅ Automated CRU-based deployment
...
│   └── test/
│
├── configurations/                # Configuration root (CRU-based)
│   │
│   ├── helm/                      # Helm charts for Kubernetes deployment
│   │   └── <service-name>/        # e.g., lcm-service, order-service
│   │       ├── main/              # Base chart (ALWAYS present)
│   │       │   ├── Chart.yaml
│   │       │   ├── values.yaml
│   │       │   └── templates/
│   │       └── <CRU-TAG>/         # Customer-specific overrides (OPTIONAL)
│   │           └── values.yaml    # e.g., CUST-ACME-R07/values.yaml
│   │
│   ├── release-note/              # Solution release notes (NOT CRU-based)
│   │   └── <SOLUTION-TAG>/        # e.g., SL-ACME-006/
│   │       └── release-notes.md
│   │
│   ├── cdc-script/                # Change Data Capture scripts
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── cdc-config.json
│   │
│   ├── sql/                       # Database migration scripts
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       ├── V001__initial.sql
│   │       └── V002__add_columns.sql
│   │
│   ├── api-doc/                   # API documentation (Swagger/OpenAPI)
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── swagger.yaml
│   │
...
│   │       └── order-workflow.json
│   │
│   ├── initial-objects/           # Initial data objects
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── seed-data.json
│   │
...
│   │       └── architecture.md
│   │
│   ├── permission-groups/         # RBAC permission groups
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── permissions.json
│   │
│   ├── profile-templates/         # User profile templates
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── templates.json
│   │
...
│   │       └── <CRU-TAG>/         # e.g., CUST-ACME-R07/
│   │           └── mysql-datasource.json
│   │
│   ├── aoa-templates/             # Area of Application templates
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── aoa-template.json
│   │
...
│   │       └── module-config.json
│   │
│   ├── multi-lingual/             # Internationalization (i18n)
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       ├── en_US.json
│   │       ├── fr_FR.json
│   │       └── es_ES.json
│   │
│   ├── base-configuration/        # Base configuration properties
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── base-config.json
│   │
│   ├── business-configuration/    # Business configuration properties
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── business-config.json
│   │
│   ├── data-api/                  # Data API configurations
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── data-api.json
│   │
│   ├── jobs/                      # Batch jobs and ETL
│   │   ├── nifi-jobs/             # Apache NiFi jobs
│   │   │   └── <CRU-TAG>/         # e.g., CUST-ACME-R07/
│   │   │       └── data-ingestion.xml
...
│   │
│   ├── naming-identity/           # Naming and identity configurations
│   │   └── <CRU-TAG>/             # e.g., CUST-ACME-R07/
│   │       └── naming-rules.json
│   │
│   ├── unit-test-report/          # Unit test reports (NO CRU - branch level)
│   │   └── <SOLUTION-TAG>/        # e.g., SL-ACME-006/
│   │       └── junit-report.xml
│   │
│   ├── qa-test-cases/             # QA test cases (NO CRU - branch level)
│   │   └── <SOLUTION-TAG>/        # e.g., SL-ACME-006/
│   │       └── test-cases.xlsx
│   │
│   ├── security-reports/          # Security scan reports (NO CRU - branch level)
...
│   │   │   └── <SOLUTION-TAG>/
│   │   └── burp/                  # Burp Suite security testing
│   │       └── <SOLUTION-TAG>/
│   │
│   └── testcases/                 # Integration test cases (NO CRU - branch level)
│       └── <SOLUTION-TAG>/        # e.g., SL-ACME-006/
...
│   │
│   ├── helm/                      # Helm charts for Kubernetes deployment
│   │   └── <service-name>/        # e.g., lcm-ui, order-ui
│   │       ├── main/              # Base chart (ALWAYS present)
│   │       │   ├── Chart.yaml
│   │       │   ├── values.yaml
│   │       │   └── templates/
...
│   │       └── <SOLUTION-TAG>/
│   │
│   └── testcases/                 # UI test cases (NO CRU - branch level)
│       └── <SOLUTION-TAG>/        # e.g., SL-ACME-006/
│           └── e2e-tests.json
│
...
for config_folder in configurations/*; do
    ...
    if [[ "$folder_name" =~ ^(unit-test-report|qa-test-cases|security-reports|testcases|release-note)$ ]]; then
        echo "⏭️  Skipping $folder_name (not CRU-based)"
        continue
    fi
    
    # Check if CRU folder exists
    if [ -d "$config_folder/$CRU_TAG" ]; then
        echo "✅ Processing $folder_name/$CRU_TAG"
        deploy_configuration "$config_folder/$CRU_TAG"
    else
        echo "⏭️  Skipping $folder_name (no CRU folder: $CRU_TAG)"
    fi
done
```
...
│       └── ingress.yaml
│
└── CUST-ACME-R07/                 # Customer-specific overrides
    └── values.yaml                # Override values only
```
...
├── helm/lcm-service/main/         ✅ Always present
├── workflow/CUST-SAMPLE-R01/      ✅ Sample for testing
└── grids/                         ✅ Empty or sample
```
...
Backend/configurations/
├── helm/lcm-service/main/         ✅ Version 2.3.0 base chart
├── sql/                           ✅ Empty (no customer yet)
└── workflow/                      ✅ Empty (no customer yet)
```
...
├── helm/lcm-service/
│   ├── main/                      ✅ Base chart
│   └── CUST-ACME-R07/             ✅ ACME production overrides
├── sql/CUST-ACME-R07/             ✅ ACME SQL migrations
├── workflow/CUST-ACME-R07/        ✅ ACME workflows
├── grids/CUST-ACME-R07/           ✅ ACME grid configs
└── release-note/SL-ACME-006/      ✅ Solution release notes
```
...
Backend/
├── src/main/java/com/acme/custom/  ✅ ACME custom code
├── configurations/
│   ├── helm/lcm-service/
│   │   ├── main/                   ✅ Base chart
│   │   └── CUST-ACME-R07/          ✅ ACME overrides
│   ├── sql/CUST-ACME-R07/          ✅ ACME SQL
│   ├── workflow/CUST-ACME-R07/     ✅ ACME workflows
│   └── api-doc/CUST-ACME-R07/      ✅ ACME API docs
```
...
|--------|---------|--------------|--------------|
| `helm/<service>/<CRU>` | Helm value overrides | ✅ Yes | `helm/lcm-service/CUST-ACME-R07/values.yaml` |
| `cdc-script/<CRU>` | CDC configurations | ✅ Yes | `cdc-script/CUST-ACME-R07/cdc.json` |
| `sql/<CRU>` | Database migrations | ✅ Yes | `sql/CUST-ACME-R07/V001__init.sql` |
| `api-doc/<CRU>` | API documentation | ✅ Yes | `api-doc/CUST-ACME-R07/swagger.yaml` |
| `workflow/<CRU>` | Workflow definitions | ✅ Yes | `workflow/CUST-ACME-R07/order.json` |
| `initial-objects/<CRU>` | Seed data | ✅ Yes | `initial-objects/CUST-ACME-R07/data.json` |
| `permission-groups/<CRU>` | RBAC permissions | ✅ Yes | `permission-groups/CUST-ACME-R07/perms.json` |
| `profile-templates/<CRU>` | User profiles | ✅ Yes | `profile-templates/CUST-ACME-R07/template.json` |
| `bi-dashboards/widget/<CRU>` | BI widgets | ✅ Yes | `bi-dashboards/widget/CUST-ACME-R07/widget.json` |
| `bi-dashboards/dashboards/<CRU>` | BI dashboards | ✅ Yes | `bi-dashboards/dashboards/CUST-ACME-R07/dash.json` |
| `bi-dashboards/datasource/<CRU>` | BI datasources | ✅ Yes | `bi-dashboards/datasource/CUST-ACME-R07/ds.json` |
| `aoa-templates/<CRU>` | AOA templates | ✅ Yes | `aoa-templates/CUST-ACME-R07/aoa.json` |
| `modules/<CRU>` | Module configs | ✅ Yes | `modules/CUST-ACME-R07/module.json` |
| `multi-lingual/<CRU>` | i18n translations | ✅ Yes | `multi-lingual/CUST-ACME-R07/en_US.json` |
| `base-configuration/<CRU>` | Base configs | ✅ Yes | `base-configuration/CUST-ACME-R07/config.json` |
| `business-configuration/<CRU>` | Business configs | ✅ Yes | `business-configuration/CUST-ACME-R07/config.json` |
| `data-api/<CRU>` | Data API configs | ✅ Yes | `data-api/CUST-ACME-R07/api.json` |
| `jobs/nifi-jobs/<CRU>` | NiFi jobs | ✅ Yes | `jobs/nifi-jobs/CUST-ACME-R07/job.xml` |
| `jobs/spark-jobs/<CRU>` | Spark jobs | ✅ Yes | `jobs/spark-jobs/CUST-ACME-R07/job.py` |
| `agents/<CRU>` | Agent configs | ✅ Yes | `agents/CUST-ACME-R07/agent.json` |
| `notifications/<CRU>` | Notification templates | ✅ Yes | `notifications/CUST-ACME-R07/email.json` |
| `naming-identity/<CRU>` | Naming rules | ✅ Yes | `naming-identity/CUST-ACME-R07/rules.json` |
| `grids/<CRU>` (UI) | Grid configurations | ✅ Yes | `grids/CUST-ACME-R07/order-grid.json` |
| `pages/<CRU>` (UI) | Page configurations | ✅ Yes | `pages/CUST-ACME-R07/dashboard.json` |
| `forms/<CRU>` (UI) | Form configurations | ✅ Yes | `forms/CUST-ACME-R07/order-form.json` |

**Pipeline Behavior:** If CRU folder **NOT** present → **SKIP** deployment
...
4. **Pipeline automatically detects and deploys**:
   ```
   ✅ Processing workflow/CUST-ACME-R07
   ✅ Deploying order-approval.json to production
   ```
...
4. **Pipeline applies override**:
   ```bash
   📦 Deploying base chart: lcm-service/main
   🔧 Applying CRU overrides: lcm-service/CUST-ACME-R07
...
if [ -d "configurations" ]; then
    for folder in configurations/*; do
        ...
        if [[ ! " ${VALID_CONFIG_FOLDERS[@]} " =~ " ${folder_name} " ]]; then
            echo "❌ ERROR: Invalid configuration folder: $folder_name"
...
1. **cru_metadata → solution_release**: One CRU can be referenced by multiple solution releases (N:1)
2. **solution_release → component_release**: One solution has many components (1:N)
3. **solution_release → env_deploy**: One solution can be deployed to multiple environments (1:N)

---

### 6.2 Table Definitions

#### Table 1: `cru_metadata`

**Purpose:** Stores metadata for Configuration Release Units (CRU). Each CRU represents a unique set of configurations (UI grids, forms, workflows, SQL scripts, etc.) for a specific customer release.

```sql
CREATE TABLE cru_metadata (
  id BIGSERIAL PRIMARY KEY,
  cru_tag VARCHAR(50) UNIQUE NOT NULL,
...
| `cru_tag` | VARCHAR(50) | Unique CRU identifier | `CUST-ACME-R07` |
| `customer` | VARCHAR(100) | Customer/tenant name | `ACME` |
| `description` | TEXT | Description of this CRU release | `Q4 2025 configuration release with new workflows` |
| `created_at` | TIMESTAMP | Creation timestamp | `2025-11-09 10:00:00` |
| `created_by` | VARCHAR(100) | User who created the CRU | `config.admin@visionwaves.com` |
| `updated_at` | TIMESTAMP | Last update timestamp | `2025-11-10 14:30:00` |
| `updated_by` | VARCHAR(100) | User who last updated the CRU | `config.admin@visionwaves.com` |

**Notes:**
- CRU configurations (grids, forms, workflows, SQL, etc.) are stored in **Git repositories** under `configurations/<config-type>/<CRU-TAG>/` folders
- This table only stores **metadata** about the CRU, not the actual configuration files
- The `cru_tag` is referenced by `solution_release.cru_tag` to link configurations to releases

---

#### Table 2: `solution_release`
...
CREATE TABLE solution_release (
  ...
  FOREIGN KEY (cru_tag) REFERENCES cru_metadata(cru_tag),
  INDEX idx_customer (customer),
...
|-------|------|-------------|---------|
| `id` | BIGSERIAL | Primary key | `1` |
| `solution_key` | VARCHAR(100) | Logical solution name | `NetsinularityOS-Core` |
| `release_tag` | VARCHAR(50) | Unique release identifier | `SL-ACME-006` |
| `customer` | VARCHAR(100) | Customer/tenant name | `ACME` |
| `cru_tag` | VARCHAR(50) | Configuration Release Unit tag (FK) | `CUST-ACME-R07` |
...
**Notes:**
- The `cru_tag` field references `cru_metadata.cru_tag` to link this solution release to its configuration set
- Multiple solution releases can reference the same CRU if they share the same configuration

---

#### Table 3: `component_release`
...
#### Example: `cru_metadata` Record

```sql
INSERT INTO cru_metadata (cru_tag, customer, description, created_by)
VALUES (
  'CUST-ACME-R07',
  'ACME',
  'Q4 2025 configuration release - includes new approval workflows, enhanced employee grid, and updated dashboards',
...
```
Backend/configurations/
├── workflow/CUST-ACME-R07/
│   └── approval-workflow.json
├── grids/CUST-ACME-R07/
│   └── employee-grid.json
├── sql/CUST-ACME-R07/
│   ├── V001__initial_schema.sql
│   └── V002__add_approval_tables.sql
└── helm/lcm-service/CUST-ACME-R07/
    └── values.yaml
```
...
### 11.4 CRU Metadata Storage

CRU metadata is stored in the `cru_metadata` table for tracking purposes. All actual configuration files (grids, forms, workflows, SQL scripts, etc.) are stored in **Git repositories** under the standardized folder structure.
...
All configuration files are stored in the repository under `configurations/<config-type>/<CRU-TAG>/`:
...
│   ├── V001__initial_schema.sql
│   └── V002__add_approval_tables.sql
├── permission-groups/CUST-ACME-R07/
│   └── permissions.json
└── helm/lcm-service/CUST-ACME-R07/
    └── values.yaml
```

**Linking CRU to Solution Release:**

```sql
INSERT INTO solution_release (
  solution_key, release_tag, customer, cru_tag, 
  product_version, customer_version, status, created_by
) VALUES (
  'NetsinularityOS-Core', 
  'SL-ACME-006', 
  'ACME', 
  'CUST-ACME-R07',  -- References cru_metadata.cru_tag
  '1.12.3', 
  '2.3.0', 
  'READY', 
  'release.manager@visionwaves.com'
...
The CRU deployment process retrieves configuration files from Git and applies them to the target environment.

**Step 1: Query CRU Metadata**

```sql
SELECT cru_tag, customer, description
FROM cru_metadata
WHERE cru_tag = 'CUST-ACME-R07';
```

**Step 2: Checkout Configuration Files from Git**
...
CRU_TAG="CUST-ACME-R07"
ENVIRONMENT="sit"
REPO_BRANCH="deploy/ACME/sit"

# Clone/pull the deployment repository
git clone https://github.com/visionwaves/backend-deployments.git
cd backend-deployments
git checkout $REPO_BRANCH

# Verify CRU folder exists
if [ ! -d "configurations/workflow/$CRU_TAG" ]; then
  echo "❌ ERROR: CRU folder not found for $CRU_TAG"
  exit 1
fi

echo "✅ CRU configurations found for $CRU_TAG"
```
...
# Deploy workflows
for workflow_file in configurations/workflow/$CRU_TAG/*.json; do
  if [ -f "$workflow_file" ]; then
    workflow_name=$(basename "$workflow_file" .json)
    echo "📋 Deploying workflow: $workflow_name"
    
    curl -X POST "$CONFIG_SERVICE_URL/workflow/deploy" \
      -H "Content-Type: application/json" \
      -d @"$workflow_file"
  fi
done
...
# Deploy grids
for grid_file in configurations/grids/$CRU_TAG/*.json; do
  if [ -f "$grid_file" ]; then
    grid_name=$(basename "$grid_file" .json)
    echo "📊 Deploying grid: $grid_name"
    
    curl -X POST "$CONFIG_SERVICE_URL/grid/deploy" \
      -H "Content-Type: application/json" \
      -d @"$grid_file"
  fi
done
...
# Deploy forms
for form_file in configurations/forms/$CRU_TAG/*.json; do
  if [ -f "$form_file" ]; then
    form_name=$(basename "$form_file" .json)
    echo "📝 Deploying form: $form_name"
    
    curl -X POST "$CONFIG_SERVICE_URL/form/deploy" \
      -H "Content-Type: application/json" \
      -d @"$form_file"
  fi
done
...
# Run SQL migrations
for sql_file in configurations/sql/$CRU_TAG/*.sql; do
  if [ -f "$sql_file" ]; then
    sql_name=$(basename "$sql_file")
    echo "🗄️  Running SQL migration: $sql_name"
    
    psql -h $DB_HOST -U $DB_USER -d $DB_NAME -f "$sql_file"
  fi
done

# Deploy Helm chart overrides
if [ -d "configurations/helm/lcm-service/$CRU_TAG" ]; then
  echo "⎈ Applying Helm overrides for lcm-service"
  
  helm upgrade lcm-service configurations/helm/lcm-service/main \
    --namespace $ENVIRONMENT \
    --values configurations/helm/lcm-service/$CRU_TAG/values.yaml
fi

echo "✅ All CRU configurations deployed successfully for $CRU_TAG"
```
...
curl -s "$CONFIG_SERVICE_URL/workflow/list?cruTag=$CRU_TAG" | jq .

# Verify grids deployed
echo "Verifying grids..."
curl -s "$CONFIG_SERVICE_URL/grid/list?cruTag=$CRU_TAG" | jq .

# Verify forms deployed
echo "Verifying forms..."
curl -s "$CONFIG_SERVICE_URL/form/list?cruTag=$CRU_TAG" | jq .

# Verify SQL migrations
echo "Verifying SQL migrations..."
psql -h $DB_HOST -U $DB_USER -d $DB_NAME -c "SELECT version FROM flyway_schema_history ORDER BY installed_rank DESC LIMIT 5;"

echo "✅ CRU deployment verification complete"
```
